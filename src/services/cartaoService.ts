...
export const cartaoService = {
  ...
  emitir: (numeroCartao: number, numeroConta: number, tipoCartao: string, limite: number) => {
  ...
  bloquear: (numeroCartao: number) => {
    return api.patch(`/cartoes/${numeroCartao}/bloquear`);
  },

  desbloquear: (numeroCartao: number) => {
    return api.patch(`/cartoes/${numeroCartao}/desbloquear`);
  },
};
